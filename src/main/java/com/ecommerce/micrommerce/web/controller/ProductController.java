...
public class ProductController {
    ...
    public ResponseEntity<Product> ajouterProduit(@RequestBody @Valid Product product) {
        if (product.getPrix() == 0) throw new ProduitGratuitException("Un produit ne peut etre gratuit");
    ...
    public Map<String, Integer> calculerMargeProduit(){
        // Créer un Map pour stocker les produits et leurs marges
        Map<String, Integer> marges = new HashMap<>();
        List<Product> products = productDao.findAll();

        for (Product prod : products){
            int marge = prod.calculerMarge();
            marges.put(prod.toString(), marge);
        }
